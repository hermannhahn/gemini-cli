...
Geminid CLI supports several built-in commands to help you manage your session, customize the interface, and control its behavior. These commands are prefixed with a forward slash (`/`), an at symbol (`@`), or an exclamation mark (`!`).
...
- **`/bug`**
  - **Description:** File an issue about Geminid CLI. By default, the issue is filed within the GitHub repository for Geminid CLI. The string you enter after `/bug` will become the headline for the bug being filed. The default `/bug` behavior can be modified using the `bugCommand` setting in your `.gemini/settings.json` files.
...
- **`/copy`**
  - **Description:** Copies the last output produced by Geminid CLI to your clipboard, for easy sharing or reuse.
...
- **`/extensions`**
  - **Description:** Lists all active extensions in the current Geminid CLI session. See [Geminid CLI Extensions](../extension.md).

- **`/help`** (or **`/?`**)
  - **Description:** Display help information about Geminid CLI, including available commands and their usage.
...
- **`/settings`**
  - **Description:** Open the settings editor to view and modify Geminid CLI settings.
  - **Details:** This command provides a user-friendly interface for changing settings that control the behavior and appearance of Geminid CLI. It is equivalent to manually editing the `.gemini/settings.json` file, but with validation and guidance to prevent errors.
...
- **`/stats`**
  - **Description:** Display detailed statistics for the current Geminid CLI session, including token usage, cached token savings (when available), and session duration. Note: Cached token information is only displayed when cached tokens are being used, which occurs with API key authentication but not with OAuth authentication at this time.

- [**`/theme`**](./themes.md)
  - **Description:** Open a dialog that lets you change the visual theme of Geminid CLI.
...
- [**`/tools`**](../tools/index.md)
  - **Description:** Display a list of tools that are currently available within Geminid CLI.
  - **Sub-commands:**
    - **`desc`** or **`descriptions`**:
...
Custom commands allow you to save and reuse your favorite or most frequently used prompts as personal shortcuts within Geminid CLI. You can create commands that are specific to a single project or commands that are available globally across all your projects, streamlining your workflow and ensuring consistency.

#### File Locations & Precedence

Geminid CLI discovers commands from two locations, loaded in a specific order:
...
When a custom command attempts to execute a shell command, Geminid CLI will now prompt you for confirmation before proceeding. This is a security measure to ensure that only intended commands can be run.
...
Geminid CLI will then execute the multi-line prompt defined in your TOML file.
...
The `!` prefix lets you interact with your system's shell directly from within Geminid CLI.
...
- **`!<shell_command>`**
  ...
  - **Examples:**
    - `!ls -la` (executes `ls -la` and returns to Geminid CLI)
    - `!git status` (executes `git status` and returns to Geminid CLI)
...
- **`!` (Toggle shell mode)**
  - **Description:** Typing `!` on its own toggles shell mode.
    ...
    - **Exiting shell mode:**
      - When exited, the UI reverts to its standard appearance and normal Geminid CLI behavior resumes.

- **Caution for all `!` usage:** Commands you execute in shell mode have the same permissions and impact as if you ran them directly in your terminal.

- **Environment Variable:** When a command is executed via `!` or in shell mode, the `GEMINI_CLI=1` environment variable is set in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the Geminid CLI.
