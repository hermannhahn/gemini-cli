...
This monorepo contains two main packages: `@hahnd/geminid` and `@hahnd/gemini-cli-core`.
...
When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@hahnd/gemini-cli-core`. This means that whether a user installs the package with `npm install -g @hahnd/geminid` or runs it directly with `npx @hahnd/geminid`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/hermannhahn/gemini-cli/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/hermannhahn/gemini-cli/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/hermannhahn/gemini-cli/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/hermannhahn/gemini-cli/pulls) of the repository.
...
- `npx -y @hahnd/geminid@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @hahnd/geminid@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @hahnd/geminid && npm uninstall -g @hahnd/geminid && npm cache clean --force &&  npm install @hahnd/geminid@<version>`
...
1.  Go to the [Actions tab](https://github.com/hermannhahn/gemini-cli/actions/workflows/release.yml) of the repository.
...
2.  The JavaScript Bundle is Created:
    ...
    - File movement: packages/cli/dist/index.js + packages/core/dist/index.js -> (bundled by esbuild) -> `bundle`/geminid.js (or a
      similar name).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @hahnd/geminid`.
