...
- **Error: Command not found (when attempting to run Gemini CLI with `gemini`).**
  ...
  - **Solution:**
    ...
    - If you installed `gemini` globally, check that your `npm` global binary directory is in your `PATH`. You can update Gemini CLI using the command `npm install -g @hahnd/geminid@latest`.
