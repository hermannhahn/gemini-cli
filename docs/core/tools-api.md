...
- **Memory Tools:**
  - `AddMemoryTool` (`stm.ts`): Adds a new entry to the Memory file.
  - `SearchStmTool` (`stm.ts`): Searches for entries in the Memory file.
  - `DeleteStmTool` (`stm.ts`): Deletes an entry from the Memory file by ID.
  - `ClearStmTool` (`stm.ts`): Clears entries from the Memory file older than 35 days.
