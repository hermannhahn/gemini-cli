...
git checkout -b hermannhahn/main # Creates your release branch
git push -u origin hermannhahn/main # Publishes your release branch to your fork
```
...
After a feature or fix is merged into `hermannhahn/develop` and a new release is desired, follow these steps:

1.  **Merge to Release Branch:** Merge `hermannhahn/develop` into `hermannhahn/main`.

    ```bash
    git checkout hermannhahn/main
    git merge hermannhahn/develop --no-ff # Merges the development branch into the release branch
