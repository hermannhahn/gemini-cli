...
```bash
git clone https://github.com/hermannhahn/gemini-cli.git # Or your fork's URL
cd gemini-cli
```
...
To start the Geminid CLI from the source code (after building), run the following command from the root directory:
...
The integration tests are designed to validate the end-to-end functionality of the Geminid CLI. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for the Geminid CLI.
...
1.  **Start the Geminid CLI in development mode:**
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Geminid CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.gemini/sandbox.Dockerfile` and/or `.gemini/sandbox.bashrc` under your project settings directory (`.gemini`) and running `gemini` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
